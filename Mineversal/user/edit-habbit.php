...
if(isset($_POST['submit'])) {
    $id = intval($_POST['id_habbit']);
    $nama_habbit = $_POST['nama'];
    $catatan = $_POST['catatan'];
    $catatan_besok = $_POST['catatan_besok'];
    $query = mysqli_query($kon, "UPDATE habit SET nama_habbit='$nama_habbit', catatan='$catatan', catatan_besok='$catatan_besok' WHERE id_habbit='$id'");
    if($query) {
...
if($_GET['id_habbit']) {
    ...
    $result = mysqli_query($kon, "SELECT * FROM habit WHERE id_habbit='$id'");
    if ($result === FALSE) {
        die(mysqli_error());
    } else {
        while($data = mysqli_fetch_array($result)) {
            $nama_habbit = $data['nama_habbit'];
            $catatan = $data['catatan'];
            $catatan_besok = $data['catatan_besok'];
        }
    }
}
...
<!DOCTYPE html>
    ...
    <head>
        ...
        <title>Mineversal | Edit Habbit</title>
        <style>
            html {
                    ...
                    <div class="container-fluid mb-5 pt-5">
                        ...
                        <form action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="POST">
                            <div class="table-responsive">
                                <input type="hidden" name="id_habbit" value="<?php echo $_GET['id_habbit'];?>">
                                <table class="table table-striped table-sm">
                                    <tbody>
                                        <tr>
                                            <td><h3>Nama Habbit</h3></td>
                                            <td><input class="my-auto TextField" type="text" name="nama" value="<?php echo $nama_habbit;?>" placeholder="Enter Nama Habbit" required/></td>
                                        </tr>
                                        <tr>
                                            <td><h3>Catatan</h3></td>
                                            <td><input class="my-auto TextField" type="text" name="catatan" value="<?php echo $catatan;?>" placeholder="Enter Catatan" required/></td>
                                        </tr>
                                        <tr>
                                            <td><h3>Catatan Besok</h3></td>
                                            <td><input class="my-auto TextField" type="text" name="catatan_besok" value="<?php echo $catatan_besok;?>" placeholder="Enter Catatan Besok" required/></td>
                                        </tr>
                                        <tr>
                                            <td></td>
