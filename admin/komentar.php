...
if( $_GET['disid']) {
	...
	$query=mysqli_query($kon, "UPDATE comments SET status='0' WHERE id='$id'");
	$msg="Komentar Batal Disetujui";
}

if($_GET['appid']) {
	$id=intval($_GET['appid']);
	$query=mysqli_query($kon, "UPDATE comments SET status='1' WHERE id='$id'");
...
if($_GET['action']=='del' && $_GET['rid']) {
	$id=intval($_GET['rid']);
	$query=mysqli_query($kon, "DELETE FROM comments WHERE id='$id'");
	$delmsg="Komentar Dihapus Selamanya!";
}
    ...
    <body>
        ...
        <div class="container-fluid">
            <div class="row">
                ...
                <main role="main" class="col-md-9 ml-sm-auto col-lg-10 px-md-4 kanan mb-5">
                    <div class="container-fluid mt-5 pt-5">
                        ...
                        <?php } ?>
                        <h2>Komentar Meminta Disetujui</h2>
                        <div class="row">
                            <div class="col-sm-12">
                                <div class="card-box">
                                    <div class="table-responsive">
                                        <table class="table table-colored table-centered table-inverse m-0 bg-dark">
                                            ...
                                            <tbody>
                                                <?php
                                                    $query=mysqli_query($kon,"SELECT comments.id as id, comments.nama as nama, comments.PostingDate as PostingDate, comments.comment as comment, post.id as postid, post.title as title FROM comments JOIN post ON post.id = comments.idpost WHERE comments.status=0");
                                                ...
                                                <tr>
                                                    ...
                                                    <td><?php $st=$row['status'];
                                                        if($st==0) {
                                                            echo "Menunggu untuk Disetujui";
                                                        } else {
                                                            echo "Disetujui";
                                                        } ?></td>
                                                    <td><a href="edit-post?id=<?php echo htmlentities($row['postid']);?>"><?php echo htmlentities($row['title']);?></a> </td>
                                                    <td><?php echo htmlentities($row['PostingDate']);?></td>
                                                    <td><?php if($st=='0') { ?>
                                                        <a href="komentar?disid=<?php echo htmlentities($row['id']);?>" title="Disapprove this comment"><i class="fa fa-pencil" style="color: #29b6f6;"></i></a> 
                                                    <?php } else { ?>
                                                        <a href="komentar?appid=<?php echo htmlentities($row['id']);?>" title="Approve this comment"><i class="fa fa-pencil" style="color: #29b6f6;"></i></a> 
                                                    <?php } ?>
                                                        &nbsp;<a href="komentar?rid=<?php echo htmlentities($row['id']);?>&&action=del"> <i class="fa fa-trash-o" style="color: #f05050"></i></a> </td>
                                                </tr>
                                                <?php } }?>
                                            </tbody>
                                        </table>
                                ...
                                <div class="card-box">
                                    <div class="table-responsive">
                                        <table class="table table-colored table-centered table-inverse m-0 bg-dark">
                                            ...
                                            <tbody>
                                                <?php
                                                    $query=mysqli_query($kon,"SELECT comments.id as id, comments.nama as nama, comments.PostingDate as PostingDate, comments.comment as comment, post.id as postid, post.title as title FROM comments JOIN post ON post.id = comments.idpost WHERE comments.status=1");
                                                    $rowcount=mysqli_num_rows($query);
                                                    if($rowcount==0){
                                                ?>
                                                <tr>
